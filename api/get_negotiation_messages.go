...
type getNegotiationMessagesAnswer struct {
	answerCommon
	Result map[string]models.NegotiationMessageT
}

func (api *api) GetNegotiationMessages(networkID, peerIDTo string) (int, map[string]models.NegotiationMessageT, error) {
	if len(networkID) == 0 {
